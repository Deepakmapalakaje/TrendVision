...
class LockFreeDatabaseManager:
    ...
    def _write_latest_candle_batch(self, batch):
        if not batch:
            logger.debug("latest_candle_batch is empty, returning")
            return
        try:
            logger.info(f"Writing {len(batch)} latest candle records to database")

            # Process each record individually to avoid transaction issues
            inserted_count = 0

            for candle_data in batch:
                try:
                    logger.debug(f"Processing candle_data for {candle_data['instrument_key']}")

                    # Create a clean tuple of 37 values
                    values = (
                        candle_data['instrument_key'],
                        candle_data['instrument_name'],
                        candle_data['instrument_type'],
                        candle_data.get('strike_price', 0) or 0,
                        candle_data.get('option_type', '') or '',
                        candle_data['timestamp'],
                        float(candle_data['open']),
                        float(candle_data['high']),
                        float(candle_data['low']),
                        float(candle_data['close']),
                        int(candle_data['volume']),
                        float(candle_data['atp'] or candle_data['close']),
                        float(candle_data.get('vwap', 0) or 0),
                        float(candle_data.get('price_change', 0) or 0),
                        float(candle_data.get('price_change_pct', 0) or 0),
                        int(candle_data.get('delta', 0) or 0),
                        float(candle_data.get('delta_pct', 0) or 0),
                        int(candle_data.get('min_delta', 0) or 0),
                        int(candle_data.get('max_delta', 0) or 0),
                        int(candle_data.get('buy_volume', 0) or 0),
                        int(candle_data.get('sell_volume', 0) or 0),
                        int(candle_data.get('tick_count', 0) or 0),
                        float(candle_data.get('vtt_open', 0) or 0),
                        float(candle_data.get('vtt_close', 0) or 0),
                        candle_data['candle_interval'],
                        int(candle_data.get('trend_value', 0) or 0),
                        candle_data.get('buy_recommendation', '') or '',
                        candle_data.get('entry_price'),
                        candle_data.get('target'),
                        candle_data.get('sl'),
                        candle_data.get('profit_loss'),
                        float(candle_data.get('prev_close', 0) or 0),
                        float(candle_data.get('intraday_high', candle_data['high']) or candle_data['high']),
                        float(candle_data.get('intraday_low', candle_data['low']) or candle_data['low']),
                        candle_data.get('last_updated'),
                        candle_data.get('updated_at', candle_data.get('last_updated'))
                    )

                    logger.debug(f"Values tuple has {len(values)} items")

                    # Execute the insert without explicit transaction
                    self.cursor.execute('''
                    ...
                    logger.error(f"Error inserting individual candle record: {e}")
                    logger.error(f"Candle data keys: {list(candle_data.keys())}")
                    continue

            # Commit all changes
            if inserted_count > 0:
                self.db_conn.commit()
                logger.info(f"Successfully inserted {inserted_count} out of {len(batch)} records into latest_candles")
...
class LockFreeTickProcessor:
    ...
    def _finalize_5min_candle(self):
        ...
        try:
            ...
            # Mark 5min candle for trend processing
            # Note: Trend analysis will be handled based on 5-minute data, not 1-minute trend
            try:
                # For 5min candles, just update the candle and don't run full trend analysis
                trend_data = {
                    'timestamp': self.current_5min_candle.timestamp,
                    'candle_interval': "5min",
                    'trend_value': 0,  # Will be determined separately
                ...
                self._update_latest_candle(self.current_5min_candle, "5min", trend_data)
            except Exception as e:
                logger.error(f"Error in 5min candle update: {e}")
    ...
    def _process_trend_and_recommendation(self, interval: str, candle: LiveCandle = None):
        ...
        # Immediately save to latest_candles when a candle is processed
        trend_data = {
            'timestamp': candle_to_process.timestamp,
            'candle_interval': interval,
            'trend_value': 0,  # Will be updated below
            'buy_recommendation': None,
            'entry_price': None,
            'target': None,
            'sl': None,
        ...
        if not index_ha or not future_ha:
            ...
            return
        # Correct trend calculation: HA_Open < HA_Close (bullish) + SAR upslope (long-term trend)
    ...
    def _update_latest_candle(self, candle: LiveCandle, interval: str, trend_data: dict):
        try:
            ...
            if not instrument_config:
                logger.debug(f"No instrument config found for {self.instrument_key}")
                return
            ...
            # Create data dictionary with all required fields (37 fields total)
            try:
                latest_candle_data = {
                    'instrument_key': self.instrument_key,
                    'instrument_name': instrument_name,
                    'instrument_type': instrument_config["type"],
                    'strike_price': instrument_config.get("strike_price") or 0,
                    'option_type': instrument_config.get("option_type") or '',
                    'timestamp': candle.timestamp,
                    'open': candle.open,
                    'high': candle.high,
                    'low': candle.low,
                    ...
                    'buy_recommendation': trend_data.get('buy_recommendation') or '',
                    ...
                    'prev_close': candle.open,  # Use open as previous close for now
                    'intraday_high': candle.high,
                    'intraday_low': candle.low,
                    'last_updated': datetime.now(IST)
                }

                # Add the missing 'updated_at' field to make exactly 37 fields
                latest_candle_data['updated_at'] = datetime.now(IST)

                logger.debug(f"Latest candle data has {len(latest_candle_data)} fields: {list(latest_candle_data.keys())}")

            except AttributeError as e:
                logger.error(f"AttributeError constructing candle data: {e}")
                return
            except Exception as e:
                logger.error(f"Error constructing candle data dictionary: {e}")
                return

            logger.debug(f"Saving latest candle data for {self.instrument_key}: {len(latest_candle_data)} fields")
