...
# Time Configuration
MARKET_END_TIME = dt_time(15, 30)  # 3:30 PM - System termination
TRADING_START_TIME = dt_time(10, 0)  # 10:00 AM - Buy recommendations start
NO_NEW_TRADES_TIME = dt_time(15, 0)  # 3:00 PM - No new trade recommendations
    ...
    def _create_candle_table_sync(self, table_name: str, instrument_key: str, interval: str = "1min"):
        try:
            ...
            self.cursor.execute('''
                ...
                VALUES (?, ?, ?, ?)
            ''', (table_name, instrument_key, data_type, date.today()))
    ...
    def process_tick(self, feed_data):
        try:
            ...
            if current_time >= MARKET_END_TIME:
                logger.info(f"🏁 Market hours ended at 3:30 PM - Finalizing last candles")
...
async def websocket_v3_connection_manager():
    ...
    while not shutdown_event.is_set() and connection_attempts < max_attempts:
            ...
            async with websockets.connect(
                ...
                while not shutdown_event.is_set():
                    ...
                    if now >= MARKET_END_TIME:
                        logger.info("🕒 Market close at 3:30 PM - Initiating shutdown")
                        shutdown_event.set()
        ...
        if not shutdown_event.is_set():
            retry_delay = min(5 * connection_attempts, 30)
